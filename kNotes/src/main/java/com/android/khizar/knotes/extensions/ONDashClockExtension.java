...
package com.android.khizar.knotes.extensions;
...
public class ONDashClockExtension extends DashClockExtension {
    ...
    protected void onUpdateData(int reason) {
        ...
        expandedTitle.append(notesCounters.get(Counters.ACTIVE).size()).append(" ").append(getString(com.android.khizar.knotes.R.string.notes).toLowerCase());
        if (reminders > 0) {
            expandedTitle.append(", ").append(reminders).append(" ").append(getString(com.android.khizar.knotes.R.string.reminders));
        }

        StringBuilder expandedBody = new StringBuilder();

		if (notesCounters.get(Counters.TODAY).size() > 0) {
			expandedBody.append(notesCounters.get(Counters.TODAY).size()).append(" ").append(getString(com.android.khizar.knotes.R.string.today)).append(":");
		...
		if (notesCounters.get(Counters.TOMORROW).size() > 0) {
			expandedBody.append(notesCounters.get(Counters.TOMORROW).size()).append(" ").append(getString(com.android.khizar.knotes.R.string.tomorrow)).append(":");
        ...
        publishUpdate(new ExtensionData()
                .visible(true)
                .icon(com.android.khizar.knotes.R.drawable.ic_stat_literal_icon)
