    ...
    public static Bitmap getBitmapFromAttachment(Context mContext, Attachment mAttachment, int width, int height) {
		...
		// Audio
            ...
            bmp = ThumbnailUtils.extractThumbnail(
                    BitmapUtils.decodeSampledBitmapFromResourceMemOpt(mContext.getResources().openRawResource(com.android.khizar.knotes.R
		...
		if (!TextUtils.isEmpty(mimeType)) {
			...
			switch (type) {
				...
				case "video":
					...
					break;
				case "audio":
					uri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" + com.android.khizar.knotes.R.raw.play);
					break;
				default:
					int drawable = "x-vcard".equals(subtype) ? com.android.khizar.knotes.R.raw.vcard : com.android.khizar.knotes.R.raw.files;
					uri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" + drawable);
					break;
			}
		} else {
			uri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" + com.android.khizar.knotes.R.raw.files);
		}
		return uri;
	}
}
